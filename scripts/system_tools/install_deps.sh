...
configure_extra_repos() {
    ...
    case $OS_TYPE in
        ...
        centos|rhel|rocky|almalinux)
            if ! rpm -q epel-release &>/dev/null; then $INSTALL_CMD epel-release >> "$LOG_FILE" 2>&1; fi
            if ! command -v docker &>/dev/null; then
...
install_pkg_list() {
    ...
    for pkg in "${pkg_array[@]}"; do
        ...
        if eval "$INSTALL_CMD $pkg" >> "$LOG_FILE" 2>&1; then :; else
            echo ""
...
interactive_menu() {
    while true; do
        ...
        echo "5. 自定义选择"
        echo "------------------------"
        # [说明] 在子脚本中，0 代表退出当前脚本，即返回 vps.sh 主菜单
        echo "0. 返回/退出"
        echo ""
